...
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"kLeaveTopNtf" object:@1];
    ...
    if ([otherGestureRecognizer.view isKindOfClass:NSClassFromString(@"WMZPageDataView")]){
        UIScrollView *scrollView = (UIScrollView *)otherGestureRecognizer.view;
        if (scrollView.contentOffset.x == 0) {
            if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
                CGPoint translation = [(UIPanGestureRecognizer *)gestureRecognizer translationInView:scrollView];
                if (fabsf(translation.y) > translation.x) {
                    return NO;
                }
            }
            return YES;
