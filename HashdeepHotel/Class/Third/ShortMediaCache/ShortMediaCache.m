...
- (NSString *)fileNameWithUrl:(NSURL *)url {
    ...
    NSString *fileName = [_urlFileNameCache objectForKey:urlString];
    if(!fileName) {
        fileName = [[self md5:urlString.stringByDeletingPathExtension] stringByAppendingPathExtension:urlString.pathExtension];
        [_urlFileNameCache setObject:fileName forKey:urlString];
    }

    /*
    NSArray *array = [url.absoluteString componentsSeparatedByString:@".mp4"];
    NSString *fileName = [_urlFileNameCache objectForKey:array.firstObject];
    if(!fileName) {
        fileName = [NSString stringWithFormat:@"%@.mp4", [self md5:array.firstObject]];
