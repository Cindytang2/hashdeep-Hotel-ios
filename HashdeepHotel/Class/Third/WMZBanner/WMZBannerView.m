...
- (void)setUp{
    ...
    [self.myCollectionV registerClass:[XLBannerCollectionViewCell class] forCellWithReuseIdentifier:NSStringFromClass([XLBannerCollectionViewCell class])];
    ...
    self.bannerControl = [[WMZBannerControl alloc]initWithFrame:CGRectMake(0, self.param.wItemSize.height-20, self.param.wItemSize.width, 20) WithModel:self.param];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.param.wMyCell) {
        ...
        //默认视图
        XLBannerCollectionViewCell *cell = (XLBannerCollectionViewCell *)[collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([XLBannerCollectionViewCell class]) forIndexPath:indexPath];
        
        cell.param = self.param;
//        cell.dic = self.resultArray[index];
...
- (void)setIconData:(UIImageView*)icon withData:(NSString *)urlStr{
    
    [icon sd_setImageWithURL:[NSURL URLWithString:urlStr] placeholderImage:self.param.wPlaceholderImage?[UIImage imageNamed:self.param.wPlaceholderImage]:nil];
}
