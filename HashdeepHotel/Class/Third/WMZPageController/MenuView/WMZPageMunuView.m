...
- (void)resetMainViewContenSize:(WMZPageNaviBtn*)btn{
    ...
    self.contentSize = CGSizeMake(maxX+15, 0);
}
...
- (void)setPropertiesWithBtn:(WMZPageNaviBtn*)btn withIndex:(NSInteger)i  withTemp:(WMZPageNaviBtn*)temp{
    ...
    if (image) {
        NSString *str1 = [image substringToIndex:5];
        if ([str1 isEqualToString:@"icon_"]) {
            [btn setImage:[UIImage imageNamed:image] forState:UIControlStateNormal];
        }else {
            [btn sd_setImageWithURL:[NSURL URLWithString:image] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                btn.imageView.contentMode = UIViewContentModeScaleAspectFit;
                btn.imageView.transform = CGAffineTransformMakeScale(0.5, 0.5);
                NSLog(@"imageView===%@",btn.imageView);
            }];
        }
    ...
    if (image&&selectImage) {
        ...
        if ([str1 isEqualToString:@"icon_"]) {
            ...
            [btn sd_setImageWithURL:[NSURL URLWithString:selectImage] forState:UIControlStateSelected completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                btn.imageView.contentMode = UIViewContentModeScaleAspectFit;
                btn.imageView.transform = CGAffineTransformMakeScale(0.5, 0.5);
                NSLog(@"imageView===%@",btn.imageView);
            }];
    ...
    if (temp) {
        ...
        marginX = [self getTitleData:titleInfo key:@"marginX"]?[[self getTitleData:titleInfo key:@"marginX"]floatValue]:15;
